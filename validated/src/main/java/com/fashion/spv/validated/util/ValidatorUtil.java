...
import com.fashion.spv.validated.config.GlobalValidatedProperties;
...
public class ValidatorUtil implements BeanFactoryAware {
    ...
    public static void checkPattern(ParameterType parameterType) {
        com.fashion.spv.validated.annotation.Pattern pattern = parameterType.getAnnotationCustom().getDeclaredAnnotation(parameterType, com.fashion.spv.validated.annotation.Pattern.class);
        if (pattern != null) {
            Object objectValue = parameterType.getValue();
            if (objectValue == null) {
